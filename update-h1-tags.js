...
async function updateAllPosts() {
    console.log('Updating H1 tags for implicator.ai articles...\n');
    ...
    const batchSize = 3; // Smaller batch size for updates
    for (let i = 0; i < articlesWithMultipleH1s.length; i += batchSize) {
        ...
        // Longer delay between batches for updates
    ...
    if (results.updated.length > 0) {
        ...
        results.updated.forEach(article => {
    ...
    if (results.skipped.length > 0) {
        ...
        results.skipped.forEach(article => {
            console.log(`⏭️  ${article.title} - ${article.reason}`);
        });
    }
...
console.log('⚠️  WARNING: This script will update all articles with multiple H1 tags.');
console.log('Make sure you have run preview-h1-fixes.js first to see what will be changed.');
